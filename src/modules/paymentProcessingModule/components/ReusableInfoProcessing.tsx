...
const ReusableInfoProcessing: React.FC<Props> = ({
  title,
  description,
  features,
  buttonText,
  ...
  const isImage = imageSrc.endsWith('.png') || imageSrc.endsWith('.svg');
    ...
    >
      ...
      <Link href={'/contact-us'}>
        <motion.div className='flex flex-row items-center gap-4' variants={itemVariants}>
          <button className='flex cursor-pointer flex-row items-center gap-2.5 rounded-[64px] border border-[#3c414c] bg-gradient-to-b from-[#151820] to-[#010101] px-8 py-4 transition-opacity hover:opacity-90'>
  ...
  const ImageSection = (
    ...
    >
      ...
      >
        <div
          className={`absolute inset-0 left-1/2 w-full -translate-x-1/2 ${
            hasBackground ? 'top-20' : 'top-0 h-[592px] w-[640px]'
          }`}
        >
          {isImage ? (
            <Image
...
export default ReusableInfoProcessing;
